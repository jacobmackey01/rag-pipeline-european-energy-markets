...
- **Chunking:** default 220 regex token-like units with 40 overlap. The original target was roughly 500 tokens, but MiniLM has a short input window; keeping chunks below that limit reduces truncation risk. Chunks preserve the original PDF text instead of rebuilding it, so figures such as `5.25%` and `2025-2026` are not mangled. Overlap is applied within each PDF page.
- **Vector store:** ChromaDB with cosine distance and persistent local storage in `data/chroma`.
- **Generation:** OpenAI Responses API. The default model is `gpt-4.1-mini`, a lower-cost model suitable for this short grounded-answer task. Override with `OPENAI_MODEL`. The default temperature is `0` so validation runs are deterministic.
...
- `refusal_plausible_absent_poland_peak_demand`: passed. The model returned exactly `Not found in the provided documents.` for an on-topic question asking for Poland's projected Winter 2025-2026 peak electricity demand in GW.
